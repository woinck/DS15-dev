...
class main1():
	...
	def available_spots(self, map_list, unit_list, source_num, move_target = (-1,-1)):
		...
		for i in range(1, s_unit.move_range+1):
			a = []
			for former_point in a_spots[i-1]:
				if former_point[1] >= map_list[former_point[0][0]][former_point[0][1]].move_consumption or s_unit.kind == basic.DRAGON_RIDER:
					for j in direction:
...
class main2():
	...
	def end_score(self, score, base, turn):
		'''结束后计算积分返回胜队，（-1表示平局）'''
		score[1] = (basic.TURN_MAX - turn) * 5 + base[1][0].life * 5
		if base[0][0].life <=0:
			return 1
		else:
			return 0
	...
	def calculation(self, command, base, whole_map, move_range, map_temple, score, unit_id):
		...
		if move_position in move_range:
			...
			if not sc:
				...
				base[j][i].attack_range = basic.TURRET_RANGE
			if whole_map[move_position[0]][move_position[1]].kind == basic.MIRROR:
							order = 0
			if not sc:
				#added by ning
...
class main6():
	...
	def end_score(self,score, base, turn):
		...
		for i in [0,1]:
			for j in range(0, len(base[i])):
				if base[i][j].life > 0 and i == 1:
					if base[i][j].kind < 6:
						score[i] += base[i][j].life * basic.BASE_SCORE
					else:
						score[i] += base[i][j].life * basic.HERO_SCORE
				else:
					score[i] += (base[i][j].life > 0)
		score[1] += (basic.TURN_MAX - turn) * 5 - score[0]
		return 1
