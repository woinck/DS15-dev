...
FIELD_EFFECT = {PLAIN:(1,0,0,0,0),
                ...
                GEAR:(1,2,0,0,0),
                MIRROR:(1,0,0,0,0)}
#(move_consumption, score, strength_up, speed_up, defence_up)
HERO_UP_LIMIT = 5
BASE_UP_LIMIT = 3
HERO_SCORE = 3
BASE_SCORE = 1
...
#(LIFE, STRENGTH, SPEED, DEFENCE, MOVE_RANGE, ATTACK_RANGE, MOVE_SPEED)
...
class Map_Basic:
    '''基本地形：平原、山地、森林、屏障、陷阱
    FIELD_EFFECT(move_consumption, score, strength_up, speed_up, defence_up)'''
    ...
    def effect(self, base, whole_map, unit_id, score):
        ...
        base[unit_id[0]][unit_id[1]].speed += FIELD_EFFECT[self.kind][3]
...
class Map_Gear(Map_Basic):
    ...
    def effect(self, base, whole_map, unit_id, score):
        if not self.on:
            for i in self.trap:
                whole_map[i[0]][i[1]] = Map_Trap(TRAP)
            #陷阱出现
            for i in self.barrier:
                if whole_map[i[0]][i[1]].kind == BARRIER:
                    whole_map[i[0]][i[1]] = Map_Basic(PLANE)
                else:
                    whole_map[i[0]][i[1]] = Map_Basic(BARRIER)
            #屏障产生或消失
            self.on = True
            score[unit_id[0]] += self.score
            return [(TRAP, x) for x in self.trap] + [(whole_map[x[0]][x[1]].kind, x) for x in self.barrier]
        else:
            return []
    ...
    def effect(self, base, whole_map, unit_id, score):
        if self.time >= TEMPLE_UP_TIME and ((w.kind < 6 and w.up < BASE_UP_LIMIT) or (w.kind > 5 and w.up < HERO_UP_LIMIT)):
            ...
            if self.up == 1:
                ...
                base[unit_id[0]][unit_id[1]].speed += 1
            elif self.up == 3:
...
class Hero(Base_Unit):
    def skill(self, base, other_id):
        ...
        if self.kind == HERO_3 and other.time == 0:
            ...
            base[other_id[0]][other_id[1]].speed += 1
